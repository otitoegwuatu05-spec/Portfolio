# Professional Portfolio & Interactive Chatbot
### Otito Egwuatu | Computer Technology Student @ Bowie State University

This repository contains my personal professional portfolio website, designed to showcase my technical skills, cybersecurity projects, and academic background. The site features a clean, professional **Navy & Cream** aesthetic and is optimized for both desktop and mobile viewing.

## 🚀 Key Features
* **Interactive AI Chatbot:** A custom-built assistant that provides instant information.
* **Mobile Connectivity:** Integrated QR Code functionality, allowing recruiters to transition from physical resumes to this digital portfolio instantly.
* **Professional Branding:** A tailored UI/UX featuring Bowie State University's academic context and a focus on IT technical skills.
* **Responsive Design:** Styled with CSS Flexbox and custom media queries for a seamless experience across devices.

## 🛠️ Technical Stack
* **Frontend:** HTML5, CSS3 (Custom variables, Flexbox, Glassmorphism)
* **Logic:** JavaScript (DOM manipulation for chatbot functionality)
* **Deployment:** GitHub Pages
* **Networking Focus:** Implementation of network auditing concepts through the "Copilot Packet Sniffer" project.

## 📁 Project Structure
* `index.html`: The core structure of the portfolio including About, Projects, and Contact sections.
* `style.css`: Custom styling focusing on a high-contrast Navy & Cream theme with interactive hover effects.
* `script.js`: Logic for the interactive chatbot assistant.
* `qr-code.png`: Functional QR link for mobile accessibility.

## 🎓 Academic & Certifications
* **Bowie State University:** B.S. in Computer Technology (Current)
* **Cisco:** CCNA Certified

